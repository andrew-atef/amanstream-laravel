...
@if ($plans->isEmpty())
    <div class="my-6 rounded-2xl bg-slate-50 border border-slate-200 p-4 text-xs text-slate-600">هذا المنتج بسعر ({{ number_format($price, 2) }} ج.م) متاح للشراء الدفع المباشر/الكاش عبر أمازون مصر.</div>
@else
    <div class="my-8 overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-md">
        <div class="p-5 text-white bg-slate-900 flex items-center justify-between">
            ...
            <a href="{{ $product?->affiliate_url }}" target="_blank" rel="nofollow sponsored noopener" class="hidden sm:inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2.5 rounded-lg transition">
                ...
                <tbody>
                    @foreach ($plans as $plan)
                        <tr class="border-b border-slate-100 hover:bg-sky-50/50 transition">
                            <td class="py-3 px-4 font-bold text-slate-900 text-xs">{{ $plan->bank->name_ar }}</td>
                            <td class="py-3 px-4 text-xs font-semibold text-slate-700">{{ $plan->months }} شهر</td>
                            <td class="py-3 px-4 font-black text-emerald-600 text-sm">{{ number_format($plan->calculateMonthlyPayment($price), 2) }} ج.م/شهر</td>
                            <td class="py-3 px-4">
