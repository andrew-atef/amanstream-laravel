...
<div class="my-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div class="border-b border-slate-100 bg-slate-50 px-6 py-3.5 flex items-center gap-2">
        ...
        <h3 class="font-bold text-slate-800 text-base">ملخص سريع: {{ $product?->title }}</h3>
    </div>
            ...
            <ul class="space-y-2.5 text-sm text-slate-700">
                @foreach ($pros as $item)
                    <li class="flex items-start gap-2">
                        <svg class="w-5 h-5 mt-0.5 shrink-0 text-emerald-500" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.2l-3.5-3.5L4 14.2 9 19l11-11-1.5-1.5L9 16.2z"/></svg>
                        <span>{{ $item }}</span>
                    </li>
                @endforeach
            </ul>
        ...
        <div class="p-5">
            ...
            <ul class="space-y-2.5 text-sm text-slate-700">
                @foreach ($cons as $item)
                    <li class="flex items-start gap-2">
                        <svg class="w-5 h-5 mt-0.5 shrink-0 text-rose-500" fill="currentColor" viewBox="0 0 24 24"><path d="M6.2 5L5 6.2 10.8 12 5 17.8 6.2 19 12 13.2 17.8 19 19 17.8 13.2 12 19 6.2 17.8 5 12 10.8 6.2 5z"/></svg>
                        <span>{{ $item }}</span>
                    </li>
                @endforeach
            </ul>
